...
# demod poll
# self.add_parameter('demod{}_poll'.format(demod),
#                     label = 'poll samples for demod \
#                     {}'.format(demod),
#                     get_cmd = partial(self._poll, 'demods',
#                                 demod-1, 'sample'),
#                     snapshot_value = False)
...
#     else:
#         unit = 'deg'
#     self.add_parameter('demod{}_poll_{}'.format(demod, demod_par),
#                         label = 'poll samples for demod \
#                         {} {}'.format(demod, demod_par),
#                         get_cmd = partial(self._poll_demod_sample,
#                                     demod-1, demod_par),
#                         snapshot_value = False)

# def _poll(self, module: str, number: int, setting:  str,
#             time: float) -> Union[float, int, str, dict]:
#     """
#     Function for polling data from an instrument node.
#     Args:
#         module (str): The module (eg. demodulator, input, output, ..)
#             we want to know the value of.
#         number (int): Module's index
#         setting (str): The module's setting to set.
#         time (float): Amount of time to poll data.
#     returns:
#         inquered data as dictionary
#     """
#
#     querystr = '/{}/{}/{}/{}'.format(self.device, module, number, setting)
#     log.debug("polling {}".format(querystr))
#
#     self.daq.subscribe(querystr)
#     datadict = self.daq.poll(time, 500, 1, False)
#     self.daq.unsubscribe(querystr)
#     return datadict
#
# def _poll_demod_sample(self, number: int, demod_par: str, time: float):
#     log.debug("polling demod {} param {}".format(number, demod_par))
#     module = 'demods'
#     setting = 'sample'
#     if demod_par not in ['x', 'y', 'R', 'phi']:
#         raise RuntimeError("Invalid demodulator parameter")
#     datadict = cast(dict, self._poll(module, number, setting, time))
#     datadict['R'] = np.abs(datadict['x'] + 1j * datadict['y'])
#     datadict['phi'] = np.angle(datadict['x'] + 1j * datadict['y'], deg=True)
#     return datadict[demod_par]